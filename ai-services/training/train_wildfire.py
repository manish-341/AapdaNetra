"""
AapdaNetra AI Service — Wildfire Model Training (v3 — Real Government Data Only)
Trains RandomForest and XGBoost on real IMD temperature / NDVI / fuel aridity data.
Sources:
  - wildfire_training_dataset.csv  (15,000 samples — IMD, MODIS/Sentinel-2 NDVI)
  - india_hazard_master_training_dataset.csv  (additional wildfire-labeled sites)
...
PRIMARY_DATA = os.path.join(DATA_DIR, "wildfire_training_dataset.csv")
MASTER_DATA  = os.path.join(DATA_DIR, "india_hazard_master_training_dataset.csv")

# Feature columns from the IMD/NDVI real dataset
FEATURE_COLS = [
    "elevation_m", "slope_deg", "south_facing_aspect_index",
    "mean_temperature_c", "annual_rainfall_mm", "fuel_aridity_index",
    "ndvi", "dist_to_road_km", "dist_to_river_km",
]
...
TARGET_COL = "wildfire_label"


def load_and_merge_data():
    """Load and merge all real government-sourced wildfire datasets."""
    frames = []

    # 1. Primary IMD/NDVI training dataset
    if os.path.exists(PRIMARY_DATA):
        df_primary = pd.read_csv(PRIMARY_DATA)
        print(f"  [+] wildfire_training_dataset.csv : {len(df_primary):,} rows")
        frames.append(df_primary)
    else:
        raise FileNotFoundError(f"Primary training data not found: {PRIMARY_DATA}")

    # 2. Master multi-hazard dataset — extract wildfire-relevant columns
    if os.path.exists(MASTER_DATA):
        ...
        if TARGET_COL in df_master.columns and len(master_cols_available) > 5:
            ...
            df_master_wf = df_master[keep_cols].copy()
            print(f"  [+] india_hazard_master (wildfire) : {len(df_master_wf):,} rows")
            frames.append(df_master_wf)
...
def prepare_features(df):
    """Prepare the feature matrix from real IMD/NDVI data."""
...
def compute_thresholds(df, feature_cols):
    """Compute percentile thresholds for the weather risk adjuster."""
    thresholds = {}
...
def train_and_compare():
    print("=" * 70)
    print("  AAPDANETRA WILDFIRE MODEL v3 — REAL GOVERNMENT DATA TRAINING")
    print("  Sources: IMD Temperature, MODIS/Sentinel-2 NDVI, Fuel Aridity")
    print("=" * 70)
    ...
    if TARGET_COL not in df.columns:
        if "wildfire" in df.columns:
            df[TARGET_COL] = df["wildfire"].astype(int)
        else:
    ...
    print(f"  Target distribution: {y.sum():,} wildfire / {(1-y).sum():,.0f} no-fire "
    ...
    report = {
        "model": "wildfire",
        "version": "v3.0",
        "dataset_source": "IMD Temperature Grids, MODIS/Sentinel-2 NDVI, Fuel Aridity Index, Copernicus DEM",
        "datasets_used": [
            "wildfire_training_dataset.csv",
            "india_hazard_master_training_dataset.csv (wildfire subset)",
        ],
    ...
    for target_dir in [MODELS_DIR, ROOT_MODELS_DIR]:
        ...
        joblib.dump(calibrated, os.path.join(target_dir, "wildfire_model.joblib"))
        joblib.dump(feature_cols, os.path.join(target_dir, "wildfire_features.joblib"))
        with open(os.path.join(target_dir, "wildfire_comparison.json"), "w", encoding="utf-8") as f:
            json.dump(report, f, indent=2)
        with open(os.path.join(target_dir, "wildfire_thresholds.json"), "w") as f:
